...
const NewAnalysisWizard: React.FC<NewAnalysisWizardProps> = ({ onCancel, onStart }) => {
  ...
  // Reverse geocode a coordinate to get location name
            ...
            <div className="flex flex-col lg:flex-row h-full gap-4 p-4">
                ...
                <div>
                  ...
                  <LocationSearch
                    onLocationSelect={(lat, lon, bounds, name) => {
                      console.log('Wizard received location:', { name, lat, lon });
                      mapRef.current?.flyTo(lat, lon, bounds);
                      setAnalysisName(`${name} Analysis`);
                    }}
                  />
              ...
              <MapView
                ...
                onAreaSelected={async (area, geojson) => {
                  ...
                  // Reverse geocode the center of the area to get a location name
                  if (geojson && geojson.geometry.type === 'Polygon') {
                    const coords = geojson.geometry.coordinates[0];
                    // Calculate center (simple average of coordinates)
            ...
            <div className="p-12 max-w-4xl mx-auto space-y-8 animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-4">
                <Input
                  ...
                  placeholder="e.g. Corn Field Q3 Analysis"
                />
              </div>
