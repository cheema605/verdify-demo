...
export default function NewAnalysisPage() {
    ...
    const [satelliteSource, setSatelliteSource] = useState('HLS');
    ...
    const getLocationName = async (lat: number, lon: number) => {
        try {
            const response = await fetch(
                ...
                { headers: { 'User-Agent': 'VerdifyDemo/1.0' } }
            );
            const data = await response.json();

            // Extract a meaningful name
            const name = data.address?.city || data.address?.town || data.address?.village ||
                data.address?.county || data.address?.state || data.name || 'Unknown Location';
            return name;
        } catch (error) {
    ...
    const handleStart = () => {
        // Pass the selected GeoJSON, analysis name, and indices to the processing page via URL params
        if (selectedGeoJSON) {
            const geojsonParam = encodeURIComponent(JSON.stringify(selectedGeoJSON));
            const nameParam = encodeURIComponent(analysisName || 'New Analysis');
            const indicesParam = encodeURIComponent(JSON.stringify(selectedIndices));
            router.push(`/dashboard/processing?geojson=${geojsonParam}&name=${nameParam}&indices=${indicesParam}`);
        } else {
        ...
        <div className="fixed inset-0 bg-white z-50 flex flex-col animate-in fade-in duration-300">
            ...
            <div className="flex-1 flex overflow-hidden">
                    ...
                    {currentStep === 1 && (
                        <div className="flex flex-col lg:flex-row h-full gap-4 p-4">
                            ...
                            <MapView
                                ...
                                onAreaSelected={async (area, geojson) => {
                                    ...
                                    if (geojson) {
                                        ...
                                        console.log('Getting location for:', centerLat, centerLon);
                                        const locationName = await getLocationName(centerLat, centerLon);
                                        console.log('Location name:', locationName);
                                        setLocationName(locationName);
                                        setAnalysisName(`${locationName} Analysis`);
                                    }
                                }}
                            />
                        </div>
                        ...
                        <div className="p-12 max-w-4xl mx-auto space-y-8 animate-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {[
                                    ...
                                    { name: 'NVI', long: 'Normalized Vegetation Index', desc: 'Enhanced vegetation monitoring with reduced atmospheric effects.' }
                                ].map((idx) => {
                ...
                <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between flex-shrink-0 bg-white">
                    ...
                    <Button
                        onClick={currentStep === 4 ? handleStart : next}
                    >
