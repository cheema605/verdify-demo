...
const Dashboard: React.FC<DashboardProps> = ({ onNewAnalysis, onViewProject }) => {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={onNewAnalysis} className="flex items-center gap-2">
      ...
      {/* Stats Cards - Removed Credits Used Card */}
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {mockProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => onViewProject(project.id)}>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {[
                { title: 'Cloud Cover Threshold', desc: 'AOI #42 has 85% cloud cover, analysis paused.', priority: 'High', color: 'text-rose-600', bg: 'bg-rose-50' },
                { title: 'New Granules Available', desc: 'Sentinel-2 L2A updated for Brazil Sector.', priority: 'Medium', color: 'text-amber-600', bg: 'bg-amber-50' },
                { title: 'Analysis Complete', desc: 'Tokyo Urban Index 2024 finished processing.', priority: 'Low', color: 'text-emerald-600', bg: 'bg-emerald-50' }
              ].map((alert, i) => (
                <div key={i} className={`p-3 rounded-xl border border-slate-100 hover:shadow-sm transition-shadow`}>
